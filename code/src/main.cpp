...
bool loadObjectsFromFile(string filename, Cloth *cloth, ClothParameters *cp, vector<CollisionObject *>* objects, int sphere_num_lat, int sphere_num_lon) {
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    if (key == CLOTH) {
      ...
      objects->push_back(p);
    } else { //THIS IS OUR TUBE, which has all the cloth params and also other stuff like radius and len
       Vector3D origin;
      double radius, friction, height;
      ...
      if (it_origin != object.end()) {
        ...
        incompleteObjectError("tube", "origin");
      }
      ...
      if (it_radius != object.end()) {
        ...
        incompleteObjectError("tube", "radius");
      }
      ...
      if (it_friction != object.end()) {
        ...
        incompleteObjectError("tube", "friction");
      }
      ...
      if (it_height != object.end()) {
        ...
        incompleteObjectError("tube", "height");
      }
      Tube *t = new Tube(origin, radius, friction, height);
      objects->push_back(t);
    }
  }
